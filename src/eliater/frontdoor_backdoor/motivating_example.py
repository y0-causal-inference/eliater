...
The single_mediator_with_multiple_confounders_nuisances case study is a variation of the "frontdoor-backdoor" graph,
where it contains an exposure variable (X), and an outcome (Y), one mediator variables (M1) on the directed
path connecting exposure to the outcome, and three observed confounders (Z1, Z2, Z3). In addition, it contains
three variables that are descendants of the mediators (R1, R2, R3). As the confounder(s) are observed, the
effect of exposure on the outcome can be estimated using Pearl's frontdoor or backdoor criterion.
"""
...
def generate(
    ...
    *,
    ...
    if Y in treatments:
        ...
        y = generator.normal(
            loc=beta0_y + z3 * beta_z3_to_y + m1 * beta_m1_to_y,  # + u2 * beta_u2_to_y,
            scale=10.0,
            size=num_samples,
        )
    ...
    if R1 in treatments:
        ...
        r1 = generator.normal(loc=loc_r1, scale=5.0, size=num_samples)
    ...
    if R2 in treatments:
        ...
        r2 = generator.normal(loc=loc_r2, scale=5.0, size=num_samples)
    ...
    if R3 in treatments:
        ...
        r3 = generator.normal(loc=loc_r3, scale=5.0, size=num_samples)
...
single_mediator_confounders_nuisance_vars_example = Example(
    name="frontdoor backdoor with one mediator, 3 confounders and 3 nuisance variables",
    reference="Sara Taheri.",
    description="This is an extension of the frontdoor_backdoor example from y0 module"
    " but with more variables directly connecting the treatment to outcome (mediator)"
    ...
    " aligned with the ones implied by the data.",
    graph=graph,
